namespace Billiard3D.Track
{
    ...
    internal class Room
    {
        ...
        private Sphere CreateSphereBarrier([NotNull] Sphere sphere)
        {
            ...
            var walls = Objects.Where(x => x is Wall).Cast<Wall>()
                .GroupBy(x => x.Corners.Select(y => Vector3D.AbsoluteValue(y - sphere.Center)).Min()).OrderBy(x => x.Key).SelectMany(x => x).Take(3)
                .ToList();
