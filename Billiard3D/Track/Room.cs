...
namespace Billiard3D.Track
{
    internal class Room
    {
        ...
        public void Start(Line startLine)
        {
            ...
            {
                var hitPoints = Objects.Select(x => (x, x.GetIntersectionPoints(in currentLine)))
                    .Where(x => x.Item2.Any())
                    .Select(x =>
            ...
            foreach (var track in Objects)
            {
                ...
                for (var index = 0; index < wall.WallLines.Count; index++)
                {
                    ...
                    var otherTopLine = otherWall.WallLines
                        .Except(new[] {new Line(wallLine.PointB, wallLine.PointA), wallLine}).Single(x =>
                            x.PointA == topLine.PointA || x.PointA == topLine.PointB || x.PointB == topLine.PointA ||
